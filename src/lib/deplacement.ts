...
export type Deplacement = {
  ...
  /** Le poids estimé du colis, à dire au client — livraison par transporteur seulement. */
  kg?: number;
};
 ...
 *  avec). Le prix imite ce qu'un vrai transporteur de colis facture : une
 *  prise en charge, un tarif au poids, un peu de route. Un colis long ou
 *  lourd (un garde-corps de plus de 2 m, par exemple) sort du gabarit d'un
 *  colis standard : il faut alors un transport « hors gabarit » ou en
 *  palette, nettement plus cher qu'un simple colis — d'où le supplément.
 *  Le poids est estimé pièce par pièce dans le catalogue (poidsColisKg). Les
 *  chiffres restent à valider par Quentin contre un vrai devis de
 *  messagerie.
 * ------------------------------------------------------------------ */
 ...
 * palette, à un tarif à part — d'où ce supplément forfaitaire.
 */
...
export function libelleLivraison(codePostal: string, locale: "fr" | "en"): string {
  return locale === "en" ? `Delivery by carrier — ${codePostal}` : `Livraison par transporteur — ${codePostal}`;
}

/** Le « produit » livraison, dans le panier : une ligne à part, au code postal du client. */

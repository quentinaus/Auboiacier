...
import { calculerDeplacement } from "@/lib/deplacement";
...
/**
 * Le prix du déplacement pour un code postal, tel que le serveur le calcule.
...
export async function GET(request: Request) {
  ...
  const resultat = await calculerDeplacement(cp.slice(0, 10));
  if (!resultat.ok) {
